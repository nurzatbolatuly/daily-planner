...
export function PlannedIncomeFormPage({ incCats, onBack, edit }) {
  const [name, setName] = useState(edit?.name || "");
  const [amt, setAmt] = useState(edit?.amount ? String(edit.amount) : "");
  const [catId, setCatId] = useState(edit?.category_id || incCats[0]?.id || "");
  ...
  saveRef.current = async () => {
    ...
    await supaUpsert("planned_incomes", row);
    onBack(true);
  };
  ...
  const del = async () => {
    ...
    try {
      await supa.delete("planned_incomes", `id=eq.${edit.id}`);
      onBack(true);
  ...
  return (
    <div style={{ minHeight: "calc(100dvh - var(--app-header-h))", background: C.monBg, color: "#fff", display: "flex", flexDirection: "column" }}>
      <PageHeader title={edit ? "Редактировать доход" : "Ожидаемый доход"} onBack={() => onBack(false)}/>
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 100px" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <CategoryPicker cats={incCats} value={catId} onChange={setCatId}/>
        </div>
        ...
        <div style={{ padding: "14px 16px", borderRadius: 12, background: C.monCard, marginBottom: 24 }}>
          ...
          <p style={{ margin: "8px 0 0", fontSize: 12, color: C.dim, lineHeight: 1.4 }}>
            После отметки «Получено» автоматически появится ожидаемый доход на тот же день следующего месяца.
          </p>
        </div>
        ...
        <ConfirmSheet
          ...
          onConfirm={del}
          title="Удалить ожидаемый доход?"
          message="Запись будет удалена. Если доход уже отмечен полученным, связанная транзакция останется."
        />
      </div>
    </div>
  );
