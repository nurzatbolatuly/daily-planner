...
export function TripExpenseFormMon({ exp, onSave, onCancel }) {
  ...
  const save = () => {
    const e = {};
    if (!label.trim()) e.label = "Enter name";
    ...
    <div style={{ background:C.monCard2, borderRadius:16, padding:16, marginBottom:8, border:`1px solid ${C.border}` }}>
      ...
      {status==="partial" && (
        <div style={{ marginBottom:12 }}>
          <FieldLabel>Already paid ({cur})</FieldLabel>
          <input value={paidAmt} onChange={e => setPaidAmt(e.target.value)} type="number" placeholder="0" style={{ width:"100%", background:"none", border:"none", borderBottom:`1px solid ${C.border}`, outline:"none", color:"#fff", fontSize:16, padding:"4px 0", boxSizing:"border-box" }}/>
        </div>
      )}
